...
DATASIZE = 1
interpacket_times = [0.04, 0.012]
padL = 100
output_loc = "tamaraw-results"
...
try:
    displace = int(sys.argv[1])
    output_loc = str(sys.argv[2])
except:
    print "python tamaraw.py displace=0,10,20 output_loc"
    sys.exit(0)
...
for trial in range(0, 100):
    ...
    padL = 0
    trial_x = trial/10 + displace
    trial_y = trial%10 + displace
    
    interpacket_times[0] = 1 / float((trial_x + 1) * (trial_x + 1))
    interpacket_times[1] = 1 / float((trial_y + 1) * (trial_y + 1))
