#Variation of cluster.py for time/bw.
#Involves changes to the learn_superseq code
...
fold = "datasantrim/"
output_loc = "cluster-time.results"
...
def learn_steps(superseq, clus):
    ...
    #i don't actually remember why the hell it's coded this way
    CLUSTER_SIZE = int(math.ceil(float(SITE_NUM)/STEP_NUM))

    steps = []

    for i in range(0, len(transmit_lens)):
        length = transmit_lens[i][0]
        t = transmit_lens[i][1]
        site = transmit_lens[i][2]
...
def pad(steps, transmitlen):
    ...
    if (plength == 0): #this means transmitlen > max(steplens)
        inc_length = steps[-1][0] * 0.2
        inc_time = steps[-1][1] * 0.2
        plength = steps[-1][0]
        ptime = steps[-1][1]
...
for trial in range(0, 1000):
    ...
    for i in range(0, len(testsizes)):
        ...
        padlen, padtime = transmitlen, transmittime
    ...
    fout.write("Parameters: " + str(MAX_TIME) + "\n")
